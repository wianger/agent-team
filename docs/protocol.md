...
Invalid actions, missing formal actions, nonzero agent exits, enabled hard timeouts, malformed transport, and Codex quota errors pause the entire chatroom with `paused: true` and `reason: "error"`. The failed member retains its diagnostic. The coordinator revokes all in-flight turns before cancelling their invocations, including conversation-only turns, writers, and acceptance checks. Queued or late replies cannot publish, confirm consensus, or advance the workflow. Writers retain their lease until cancellation cleanup finishes. Failed and interrupted private-session references are invalidated; committed history, documents, and filesystem side effects are preserved, not rolled back. Unexpected check-runner exceptions also pause the team; nonzero check results (including launch errors and configured check timeouts) still reopen shared work as described above.

Confirmed Claude usage exhaustion is the exception, in both chatroom and serial modes. Only that member becomes unavailable; peers' in-flight turns remain valid. Available members may continue discussion and approved work, including taking over an unavailable preferred writer, but absent members' consensus and review votes are never waived. When a required reviewer is unavailable, formal progress waits; chatroom peers can still research in the chat lane. Generic HTTP 429/capacity errors, quota warnings, and ordinary assistant text are not automatically classified as usage exhaustion. Codex's structured `UsageLimitExceeded` and Claude's native error plus rejected quota metadata are recognized, with explicit usage-exhaustion messages as a fallback.

Each native quota rejection records a durable `agent.quota` event: `{speaker, quota: {backend, error, retry_at}}`, where `retry_at` is a UTC Unix timestamp for Claude and `null` for Codex. `quota: null` clears the record. `state.quotas` also reports whether a retry is pending/in flight. Claude's deadline is detection time plus 18,000 seconds, not a claim about the provider's actual reset. An event-loop timer wakes the scheduler at that deadline without model polling or a connected human. A retry uses the latest public context and normal workflow scheduling; failure starts a fresh 5-hour cooldown only if it is again a quota error. Other retry errors pause the whole team. A successful invocation clears the quota state.

Deadlines survive server restart; recovered rooms still require `/resume`. Automatic retries never override manual pause, a Codex limit, or another blocking error. `/resume` restores Codex after a quota pause but preserves any unexpired Claude cooldown; `/retry [agent]` explicitly requests an earlier attempt. Targeted recovery cannot bypass another blocking failure. Closing the server cancels its timer without clearing the persisted deadline.

Pending work is not automatically replayed. After resolving the issue and waiting for active turns to stop, the `retry` control accepts an optional agent target (`/retry [agent]`); `/resume` clears all failures. A targeted retry cannot resume the team while another failure remains, and `/next agent` cannot bypass a different unavailable member. Human messages, redirects, and private-session resets do not clear failures. Required votes are never waived. No provider reset time is inferred and no automatic quota polling is performed. Serial mode also pauses the entire room on these failures. A stale concurrent action is a rejected decision, not a transport failure.
